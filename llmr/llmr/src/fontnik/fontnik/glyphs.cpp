...
#include <fontnik/mapnik/text/face.hpp>
...
//    mutable_fontstack->set_range(range);
    ...
    for (std::vector<uint32_t>::size_type i = range.first; i <= range.second; i++) {
        FT_ULong char_code = i;
...
//            }
            ...
            SDFGlyph SDFglyph;
                    ...
                    SDFglyph.bitmap = glyph.bitmap;

                    SDFglyph.metrics.width = glyph.width;

                    SDFglyph.metrics.height = glyph.height;

                    SDFglyph.metrics.left = glyph.left;

                    SDFglyph.metrics.top = glyph.top - glyph.ascender;

                    SDFglyph.metrics.advance = glyph.advance;

            
            stack.insert(SDFglyph.id, SDFglyph);
