...
namespace detail {
const std::regex tokenRegex("\\{(\\w+)\\}");
const std::sregex_token_iterator tokensEnd = std::sregex_token_iterator();
}
...
std::string replaceTokens(const std::string &source, const Lookup &lookup) {
    ...
    bool token = false;
    for (auto token_it = std::sregex_token_iterator(source.begin(), source.end(),
